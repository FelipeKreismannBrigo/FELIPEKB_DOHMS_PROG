...
def calcular_potencia():
    ...
    resultado_potencia = resultado ** 2
    calculo = str(resultado_potencia)
    texto.delete(1.0, "end")
    texto.insert(1.0, calculo)
...
def calcular_inverso():
    global calculo
    resultado = eval(calculo)
    resultado_inverso = 1 / resultado
    calculo = str(resultado_inverso)
    texto.delete(1.0, "end")
    texto.insert(1.0, calculo)
...
botao_potencia = tk.Button(janela, text="^2", command=calcular_potencia, width=13, height=4, font=("Arial", 12))
botao_potencia.grid(column=1, row=6)

botao_raiz = tk.Button(janela, text="√", command=calcular_raiz, width=27, height=4, font=("Arial", 12))
botao_raiz.grid(column=2, row=6, columnspan=2)

botao_inverso = tk.Button(janela, text="1/x", command=calcular_inverso, width=13, height=4, font=("Arial", 12))
botao_inverso.grid(column=4, row=6)
